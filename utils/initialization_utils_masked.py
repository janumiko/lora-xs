...
class MaskedLinear(torch.nn.Linear):
    ...
    def generate_random_mask(self, submatrix_size):
        mask = torch.zeros(self.size, self.size)
        mask[:submatrix_size, :submatrix_size] = 1
        return mask

    def set_mask_size(self, mask_size):
        self.mask_size = mask_size

    def reset_mask_size(self):
        self.mask_size = None
    ...
    def forward(self, input):
        ...
        mask = self.generate_random_mask(mask_size)
        mask = mask.to(self.weight.device)
        masked_weight = self.weight * mask
        return torch.nn.functional.linear(input, masked_weight, self.bias)


def set_dynamic_masking_size(model, mask_size):
    for name, module in model.named_modules():
        if isinstance(module, MaskedLinear):
            module.set_mask_size(mask_size)


def reset_dynamic_masking_size(model):
    for name, module in model.named_modules():
        if isinstance(module, MaskedLinear):
            module.reset_mask_size()
...
def find_and_initialize(model, peft_config, adapter_name, reconstr_type, reconstruct_config, writer):
    ...
    masked_training = reconstruct_config.get(
        "masked_training", False
    )  # randomly masking the r_squared matrix during training
    if masked_training:
        print("Using masked training.")
            ...
            if reconstruction_mode == 'separated':
                ...
                if not isinstance(target, peft.tuners.lora.Linear):
                    ...
                    if r_squared:
                        ...
                        if masked_training:
                            target.default_lora_latent_mapping = MaskedLinear(
